...
function CashierPage() {
  ...
  async function checkout() {
    ...
    const { data, error } = await supabase.rpc("create_pos_order", {
      ...
      p_paid_amount: cash,
      p_note: JSON.stringify({ orderType, customerName: customerName.trim() || null, customerPhone: customerPhone.trim() || null, discount, discountType, discountValue }),
    });
    setSubmitting(false);
  ...
  return <AppLayout>
    ...
    <div className="grid gap-6 px-5 pb-28 pt-5 lg:px-8 lg:pb-10 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <CartPanel cart={cart} subtotal={subtotal} discount={discount} discountValue={discountValue} discountType={discountType} tax={tax} total={total} cash={cash} change={change} customerName={customerName} customerPhone={customerPhone} paymentMethod={paymentMethod} orderType={orderType} setOrderType={setOrderType} setCash={setCash} setCustomerName={setCustomerName} setCustomerPhone={setCustomerPhone} setDiscountValue={setDiscountValue} setDiscountType={setDiscountType} setPaymentMethod={setPaymentMethod} updateQty={updateQty} removeFromCart={removeFromCart} checkout={checkout} submitting={submitting} />
    </div>
    {showCart && <div className="fixed inset-0 z-50 bg-brand/40 p-4 backdrop-blur-sm xl:hidden"><div className="ml-auto h-full max-w-md overflow-y-auto rounded-3xl bg-[#f7f7fa] p-4"><div className="mb-3 flex items-center justify-between"><h2 className="font-display text-xl font-bold text-brand">Current Order</h2><button onClick={() => setShowCart(false)} className="rounded-xl bg-white p-2"><X size={18} /></button></div><CartPanel cart={cart} subtotal={subtotal} discount={discount} discountValue={discountValue} discountType={discountType} tax={tax} total={total} cash={cash} change={change} customerName={customerName} customerPhone={customerPhone} paymentMethod={paymentMethod} orderType={orderType} setOrderType={setOrderType} setCash={setCash} setCustomerName={setCustomerName} setCustomerPhone={setCustomerPhone} setDiscountValue={setDiscountValue} setDiscountType={setDiscountType} setPaymentMethod={setPaymentMethod} updateQty={updateQty} removeFromCart={removeFromCart} checkout={checkout} submitting={submitting} /></div></div>}
  </AppLayout>;
}

function CartPanel(props: any) {
  return <aside className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-gray-100"><CartContent {...props} /></aside>;
}
function CartContent({ cart, subtotal, discount, discountValue, discountType, tax, total, cash, change, customerName, customerPhone, paymentMethod, orderType, setOrderType, setCash, setCustomerName, setCustomerPhone, setDiscountValue, setDiscountType, setPaymentMethod, updateQty, removeFromCart, checkout, submitting }: any) {
  const paymentOptions = [{ label: "Tunai", value: "cash" }, { label: "Kartu", value: "card" }, { label: "Transfer", value: "transfer" }, { label: "QRIS", value: "qris" }];
  return <div><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-widest text-gray-400">Order baru</p><h2 className="font-display text-xl font-bold text-brand">Current Order</h2></div><span className="rounded-full bg-brand-soft px-3 py-1 text-xs font-bold text-brand">{cart.length} item</span></div><div className="mt-4 grid gap-2"><input value={customerName} onChange={(event) => setCustomerName(event.target.value)} className="rounded-xl border border-gray-200 px-3 py-3 text-sm outline-none focus:border-brand" placeholder="Nama Customer (opsional)" /><input value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} className="rounded-xl border border-gray-200 px-3 py-3 text-sm outline-none focus:border-brand" placeholder="No. HP (untuk WA / e-struk)" inputMode="tel" /></div><div className="mt-4 grid grid-cols-3 gap-2">{["Dine In", "Take Away", "Delivery"].map((type) => <button key={type} onClick={() => setOrderType(type)} className={`rounded-xl py-2 text-[11px] font-bold ${orderType === type ? "bg-accent-yellow text-brand" : "bg-gray-50 text-gray-500"}`}>{type}</button>)}</div><div className="mt-5 flex min-h-40 flex-col gap-3">{cart.length ? cart.map((item: any) => <div key={item.product.id} className="flex gap-3 border-b border-gray-100 pb-3"><img src={item.product.foto_url ?? "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=100"} alt="" className="size-12 rounded-xl object-cover" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-brand">{item.product.nama}</p><p className="text-xs text-gray-400">{rupiah(item.product.harga)}</p><div className="mt-2 flex items-center gap-2"><button onClick={() => updateQty(item.product.id, item.qty - 1)} className="rounded-lg bg-gray-100 p-1"><Minus size={12} /></button><span className="w-5 text-center text-xs font-bold">{item.qty}</span><button onClick={() => updateQty(item.product.id, item.qty + 1)} className="rounded-lg bg-brand p-1 text-white"><Plus size={12} /></button></div></div><button onClick={() => removeFromCart(item.product.id)} className="self-start text-gray-300 hover:text-rose-500"><Trash2 size={15} /></button></div>) : <div className="grid flex-1 place-items-center rounded-2xl border border-dashed border-gray-200 py-12 text-center text-sm text-gray-400">Belum ada item.<br />Pilih menu untuk memulai.</div>}</div><div className="mt-5 border-t border-gray-100 pt-4 text-sm"><div className="flex justify-between"><span className="text-gray-500">Subtotal</span><span className="font-bold text-brand">{rupiah(subtotal)}</span></div><div className="mt-3 flex items-center justify-between gap-2"><span className="text-gray-500">Diskon</span><div className="flex gap-2"><input type="number" min="0" value={discountValue || ""} onChange={(event) => setDiscountValue(Number(event.target.value))} className="w-24 rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none" placeholder="0" /><select value={discountType} onChange={(event) => setDiscountType(event.target.value as "amount" | "percent")} className="rounded-xl border border-gray-200 px-2 py-2 text-sm"><option value="amount">Rp</option><option value="percent">%</option></select></div></div><div className="mt-2 flex justify-between"><span className="text-gray-500">Pajak</span><span className="font-bold text-brand">{rupiah(tax)}</span></div><div className="mt-4 flex justify-between border-t border-gray-100 pt-4"><span className="font-bold text-brand">TOTAL</span><span className="font-display text-2xl font-bold text-blue-600">{rupiah(total)}</span></div><div className="mt-4 grid grid-cols-2 gap-2"><p className="col-span-2 text-xs font-bold text-gray-500">Metode Pembayaran</p>{paymentOptions.map((method) => <button type="button" key={method.value} onClick={() => setPaymentMethod(method.value)} className={`rounded-xl border py-2.5 text-sm font-semibold ${paymentMethod === method.value ? "border-blue-600 bg-blue-600 text-white" : "border-gray-200 text-brand"}`}>{method.label}</button>)}</div><label className="mt-4 block text-xs font-bold text-gray-500">Jumlah Bayar<input type="number" min="0" value={cash || ""} onChange={(event) => setCash(Number(event.target.value))} className="mt-2 w-full rounded-xl border border-gray-200 px-3 py-3 text-sm outline-none" placeholder="0" /></label><div className="mt-2 flex justify-between text-sm"><span className="text-gray-500">Kembalian</span><span className="font-bold text-emerald-600">{rupiah(change)}</span></div><div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => { setCash(0); setCustomerName(""); setCustomerPhone(""); setDiscountValue(0); }} className="rounded-2xl bg-gray-100 py-3.5 text-sm font-bold text-brand">Batal</button><button disabled={submitting} onClick={checkout} className="rounded-2xl bg-emerald-600 py-3.5 text-sm font-bold text-white disabled:opacity-50">{submitting ? "Menyimpan..." : "Bayar & Cetak"}</button></div></div><div className="mt-5 border-t border-gray-100 pt-4 text-sm"><div className="flex justify-between text-gray-500"><span>Subtotal</span><span>{rupiah(subtotal)}</span></div><div className="mt-2 flex justify-between text-gray-500"><span>Pajak 10%</span><span>{rupiah(tax)}</span></div><div className="mt-3 flex justify-between text-lg font-bold text-brand"><span>Total</span><span>{rupiah(total)}</span></div></div><label className="mt-4 block text-xs font-bold text-brand">Cash diterima<input type="number" value={cash || ""} onChange={(event) => setCash(Number(event.target.value))} className="mt-2 w-full rounded-xl bg-gray-50 px-3 py-3 text-sm outline-none ring-1 ring-gray-100 focus:ring-brand" placeholder="Masukkan nominal" /></label><div className="mt-3 flex justify-between rounded-xl bg-emerald-50 px-3 py-3 text-sm font-bold text-emerald-700"><span>Kembalian</span><span>{rupiah(change)}</span></div><button disabled={submitting || !cart.length} onClick={checkout} className="mt-4 w-full rounded-xl bg-brand py-3.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{submitting ? "Memproses..." : "Bayar Cash & Cetak Struk"}</button></div>;
}
